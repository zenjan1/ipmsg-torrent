...
async fn test_download_manager_lifecycle() {
    ...
    // Add a URL download
    let task_id = manager.add_url("https://httpbin.org/robots.txt").await.unwrap();
    ...
    let tasks = manager.list_tasks().await;
    assert!(matches!(tasks[0].state, DownloadState::Queued | DownloadState::Downloading));
...
async fn test_download_manager_multiple_tasks() {
    ...
    // Add multiple tasks
    let id1 = manager.add_url("https://httpbin.org/robots.txt").await.unwrap();
    let id2 = manager.add_url("https://httpbin.org/headers").await.unwrap();
    ...
    let tasks = manager.list_tasks().await;
    let paused_count = tasks.iter().filter(|t| t.state == DownloadState::Paused).count();
...
async fn test_download_manager_dashboard() {
    ...
    // Add some tasks
    manager.add_url("https://httpbin.org/robots.txt").await.unwrap();
    manager.add_url("https://httpbin.org/headers").await.unwrap();
